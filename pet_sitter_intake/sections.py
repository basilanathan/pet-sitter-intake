...
def build_home_access_section(s, fillable=False, theme_colors=None):
    ...
    """
    elements = []
    elements += [sec_hdr("HOME ACCESS & PROPERTY INFORMATION", s, theme_colors=theme_colors), sp(0.2)]
    ...
    if fillable:
        elements.append(FillableCheckboxRow("entry_method", 
            ...
            per_row=3, theme_colors=theme_colors
        ))
    else:
    ...
    elements += field("Lockbox / key location", s, fillable=fillable, field_name="key_location", theme_colors=theme_colors)
    elements += field("Entry code(s)", s, fillable=fillable, field_name="entry_codes", theme_colors=theme_colors)
    elements += field("Alarm code & disarm instructions", s, fillable=fillable, field_name="alarm_code", multiline=True, theme_colors=theme_colors)
    elements.append(sp(0.12))
    
    elements.append(para_row("Property Details", s["sublbl"]))
    elements.append(sp(0.1))
    elements += field("WiFi network & password", s, fillable=fillable, field_name="wifi_info", theme_colors=theme_colors)
    elements += field("Parking instructions", s, fillable=fillable, field_name="parking", multiline=True, theme_colors=theme_colors)
    elements += field("Thermostat / HVAC notes", s, fillable=fillable, field_name="thermostat", multiline=True, theme_colors=theme_colors)
    elements += field("Off-limits rooms or areas", s, fillable=fillable, field_name="off_limits", theme_colors=theme_colors)
    elements += field("Other house rules (TV on for pet, mail, plants, etc.)", s, extra_lines=2, 
                      fillable=fillable, field_name="house_rules", theme_colors=theme_colors)
    
    return elements


def build_pet_behavior_section(s, pet_num=1, fillable=False, theme_colors=None):
    ...
    elements += [sec_hdr("BEHAVIOR & TEMPERAMENT", s, theme_colors=theme_colors), sp(0.2)]
    ...
    if fillable:
        elements.append(FillableCheckboxRow(f"{prefix}good_with",
            ...
            per_row=5, theme_colors=theme_colors
        ))
    else:
        elements.append(CheckboxRow(
            ["Strangers", "Children", "Other dogs", "Cats", "Small animals"],
    ...
    elements += field("Other triggers or sensitivities", s, fillable=fillable, 
                      field_name=f"{prefix}other_triggers", multiline=True, theme_colors=theme_colors)
    ...
    elements += field("Commands pet knows (sit, stay, come, etc.)", s, fillable=fillable,
                      field_name=f"{prefix}commands", theme_colors=theme_colors)
    elements += field("Recall reliability off-leash (1-10, or N/A)", s, fillable=fillable,
                      field_name=f"{prefix}recall", theme_colors=theme_colors)
    
    return elements


def build_potty_section(s, pet_num=1, fillable=False, theme_colors=None):
    ...
    elements += field("Potty schedule / signals they give when needing to go", s, fillable=fillable,
                      field_name=f"{prefix}potty_schedule", multiline=True, theme_colors=theme_colors)
    elements += field("Preferred potty area (backyard spot, specific walk route, etc.)", s, 
                      fillable=fillable, field_name=f"{prefix}potty_area", multiline=True, theme_colors=theme_colors)
    
    return elements


def build_sleep_crate_section(s, pet_num=1, fillable=False, theme_colors=None):
    ...
    elements += field("Crate location / bedtime routine", s, fillable=fillable,
                      field_name=f"{prefix}bedtime_routine", multiline=True, theme_colors=theme_colors)
    
    return elements


def build_service_specific_section(s, service_type, fillable=False, theme_colors=None):
    ...
    if service_type == "walking":
        elements += [sec_hdr("DOG WALKING DETAILS", s, theme_colors=theme_colors), sp(0.2)]
        elements += field("Leash & harness location", s, fillable=fillable, field_name="leash_location", theme_colors=theme_colors)
        elements += field("Poop bag location", s, fillable=fillable, field_name="poop_bags", theme_colors=theme_colors)
        ...
        elements += field("Preferred walking route / areas to avoid", s, fillable=fillable,
                          field_name="walk_route", multiline=True, theme_colors=theme_colors)
        elements += field("Walk duration preference", s, fillable=fillable, field_name="walk_duration", theme_colors=theme_colors)
        
    elif service_type == "boarding":
        elements += [sec_hdr("BOARDING DETAILS", s, theme_colors=theme_colors), sp(0.2)]
        ...
        elements += field("Drop-off date & time", s, fillable=fillable, field_name="dropoff_datetime", theme_colors=theme_colors)
        elements += field("Pick-up date & time", s, fillable=fillable, field_name="pickup_datetime", theme_colors=theme_colors)
        elements += field("Special items or comfort objects", s, fillable=fillable, 
                          field_name="comfort_items", theme_colors=theme_colors)
        
    elif service_type == "drop_in":
        elements += [sec_hdr("DROP-IN VISIT DETAILS", s, theme_colors=theme_colors), sp(0.2)]
        ...
        elements += field("Visit time preference", s, fillable=fillable, field_name="visit_time", theme_colors=theme_colors)
        elements += field("Visit duration needed", s, fillable=fillable, field_name="visit_duration", theme_colors=theme_colors)
